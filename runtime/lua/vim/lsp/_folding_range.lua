...
---
--- tree-sitter language of the buffer, used for foldtext highlights.
---@field lang? string
---
...
local function spans_to_virt_text(line, spans)
  ...
  vim.list.unique(boundaries)
  table.sort(boundaries)

  local virt_text = {} ---@type [string, string[]][]
  for i = 1, #boundaries - 1 do
    local start_col = boundaries[i]
    local end_col = boundaries[i + 1]
...
function M.foldtext(lnum)
  ...
  return spans_to_virt_text(line, spans)
end
